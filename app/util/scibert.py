...
def get_batched_text_outputs(device, descriptions, text_tokenizer, text_model, max_seq_len, file_type="none", batch_size=16, is_load_saved=False, is_save=False):
    ...
    # Assuming text_model output is a tensor (e.g., output['last_hidden_state'] or similar)
    # Adjust if your model returns a different structure
    #pdb.set_trace()
    #sconcatenated_output = torch.cat(all_outputs, dim=0)
    if (is_save):
