...
class TwoGraphCL(torch.nn.Module):
    def __init__(self, in_channels, hidden_dim, latent_dim, out_channels):
        super(TwoGraphCL, self).__init__()
        self.encoder1 = SmilesEncoder(in_channels, hidden_dim, latent_dim)
        self.encoder2 = SelfiesEncoder(in_channels, hidden_dim, latent_dim)
...
def train(train_loader, val_loader, train_repr, epochs=1, batch_size=64):
    ...
    model = TwoGraphCL(in_channels=input_dim, hidden_dim=hidden_dim, latent_dim=embedding_dim, out_channels=input_dim).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            smiles, selfies = batch 
            smiles.to(device)
            selfies.to(device)
            optimizer.zero_grad()
            z  = model(smiles.x, smiles.edge_index, smiles.batch, smiles.edge_attr, selfies.x, selfies.edge_index, selfies.batch)
